...
export function JobDetailHeaderCard({ selectedJob, selectedJobLabel }: JobDetailHeaderCardProps) {
  return (
    <div className={cn("min-w-0 rounded-xl border border-slate-700/80 bg-slate-950/40 p-3")}>
      <p className={cn("text-[11px] font-semibold tracking-[0.08em] text-slate-400 uppercase")}>{selectedJobLabel}</p>
      <p className={cn("mt-1 overflow-hidden break-all text-lg font-semibold text-slate-100")}>{selectedJob.command}</p>
