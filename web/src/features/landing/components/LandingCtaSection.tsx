...
export default function LandingCtaSection({ texts }: LandingCtaSectionProps) {
  return (
    <section id="docs" className={cn("mx-auto w-full max-w-5xl px-4 py-16 md:px-8")}>
      <div
        className={cn(
          "rounded-3xl border border-cyan-400/20 bg-gradient-to-br from-cyan-400/20 to-rose-400/10 px-6 py-12 text-center",
        )}
      >
        <h3 className={cn("text-4xl font-bold text-white")}>{texts.landing_cta_title}</h3>
        <p className={cn("mx-auto mt-4 max-w-2xl text-slate-300")}>{texts.landing_cta_subtitle}</p>
