...
export default function LandingPainSection({ texts }: LandingPainSectionProps) {
  return (
    <section id="how-it-works" className={cn("border-y border-[#1e2937] bg-[#0b111d] py-16")}>
      <div className={cn("mx-auto w-full max-w-6xl px-4 md:px-8")}>
        ...
        <h2 className={cn("mt-3 text-4xl font-bold text-white")}>{texts.landing_pain_title}</h2>
        <p className={cn("mt-3 max-w-2xl text-sm text-slate-400")}>{texts.landing_pain_subtitle}</p>
        <div className={cn("mt-10 grid gap-5 md:grid-cols-3")}>
          {painCards.map((card) => {
            const Icon = icons[card.icon];
            return (
              <article key={card.titleKey} className={cn("rounded-xl border border-[#2c3947] bg-[#151c26] p-6")}>
                <div
                  className={cn(
                ...
                <p className={cn("mt-3 text-sm leading-relaxed text-slate-400")}>{texts[card.bodyKey]}</p>
              </article>
            );
          })}
        </div>
