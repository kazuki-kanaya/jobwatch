...
- Apply（依存順）: `task infra:prod:apply`
- Pages の成果物を配布: `task infra:prod:deploy:pages`
- Apply + 配布: `task infra:prod:deploy`
- Destroy（逆順）: `task infra:prod:destroy`

`task infra:prod:apply` の最後に `task infra:prod:sync-dot-env` が実行され、Terraform の outputs から web/api の env が同期されます。
...
- Pages へのアップロードは `task infra:prod:apply` 後に `scripts/cloudflare/deploy-pages.sh` で実行します。
